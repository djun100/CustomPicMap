...
/** {@link #resetToInitialState()} 初始缩放为1<br/>
 * {@link #TouchImageView() onScale()} line153 阻止缩放操作*/
	...
	public TouchImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		final TouchGestureDetector.OnTouchGestureListener listener = new TouchGestureDetector.OnTouchGestureListener() {
			...
			public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
				...
				Log.w("onScroll currentDrawableWidth:"+currentDrawableWidth+" currentDrawableHeight:"+currentDrawableHeight);
...
//				if (true) return true;//refuse scale
				loadMatrixValues();
				...
				Log.e("focusX:"+focusX+" focusY:"+focusY);
	...
	private void resetToInitialState() {
		mMatrix.reset();
		final float minScale = 1f;//初始缩放为1
//		final float minScale = getMinScale();
	...
	/**由matrix获取大图当前左顶点坐标，可能在屏幕外*/
	public void loadMatrixValues() {
		mMatrix.getValues(mMatrixValues);
		mScale = mMatrixValues[Matrix.MSCALE_X];
		//总图的左上角坐标(mTranslationX,mTranslationY),可能在屏幕外。
		// 转换为以大图建立坐标系，则屏幕原点坐标为(-mTranslationX,-mTranslationY)
		mTranslationX = mMatrixValues[Matrix.MTRANS_X];
		mTranslationY = mMatrixValues[Matrix.MTRANS_Y];
		Log.w("mScale:"+mScale);
		Log.w("loadMatrixValues:mTranslationX:"+mTranslationX+" mTranslationY:"+mTranslationY);
		if (mIOnScroll!=null){
			mIOnScroll.onScroll(mMatrix);
		}
	}
	...
	public void setOnscrollListener(IOnScroll iOnScroll){
		this.mIOnScroll =iOnScroll;
	}
	public interface IOnScroll{
		void onScroll(Matrix matrix);
